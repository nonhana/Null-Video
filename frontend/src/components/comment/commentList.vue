<template>
    <div class="comment-list" v-for="comment in props.commentList" :key="comment.comment_id" :style="{
            ...
            <div class="comment-content">
                ...
                <div>
                    ...
                    <div class="comment-operate">
                        ...
                        <div class="comment-delete" v-if="comment.user.id" @click="deleteComment(comment.comment_id)">删除
                        </div>
                    </div>
                </div>
        ...
        <div class="show-comment-children" v-if="!comment.isChild && comment.children.length"
            @click="showCommentChildren.set(comment.comment_id, !showCommentChildren.get(comment.comment_id))">
            <div v-if="!showCommentChildren.get(comment.comment_id)">
                <img src="@/assets/svgs/arrow-down.svg" alt="" style="transform: scale(0.8);">
                <span>展开 {{ comment.children.length }} 条回复</span>
            </div>
            <div v-if="showCommentChildren.get(comment.comment_id)">
                <img src="@/assets/svgs/arrow-up.svg" alt="" style="transform: scale(0.8);">
                <span>收起</span>
            </div>
        </div>
